...
from manimlib.utils.bezier import bezier, split_bezier_points
...
from manimlib.utils.space_ops import normalize, interslice
...
class VMobject(Mobject):
    ...
    if 1 == 1:
        ...
        def get_stroke(self, background=False):
            return [self.get_stroke_colors(), self.get_stroke_width(), self.get_stroke_opacity()]
        ...
        def match_background_image_file(self, vmobject):
            self.color_using_background_image(
                vmobject.get_background_image_file())
            return self
    ...
    def set_points(self, points):
        '''set points<points>(%nppcc=n) to VMobj.points'''
        self.points = self.validity_of_shape_input(points)
        return self

    def set_points_as_path(self, points):
        '''set paths<points>(%nppcc=0) to VMobj.points'''
        self.points = self.validity_of_shape_input(points, 'path')
        return self

    def set_points_as_corners(self, points):
        '''set lines ctrls from ends<points> to VMobj.points
        ie convert:[end,...]-->[anchor1, handle1, handle2, anchor2,...]'''
        self.set_points_as_path(self.alpha_pts(points))
        return self

    def set_anchors_and_handles(self, anchors1, handles1, handles2, anchors2):
        '''set four sets of ctrls<anchors1, handles1, handles2, anchors2> 
            to VMobj.points
        ie convert:[anchor1s],[handle1s], [handle2s], [anchor2s]
        ...
        self.set_points_as_path(self.pts_fm_4ctrls(anchors1, handles1, handles2, anchors2))
        return self
    def set_points_as_subpaths(self, points,length=3):
        '''add lines pts from subpaths<points>(n, p, 3) to VMobj.points
        ie lines pts:[anchor1, handle1, handle2, anchor2,*line,...]'''
        self.clear_points()
        self.add_points_as_subpaths(points,length)
        return self

    # add point
    def start_new_path(self, point):
        '''append firstend<point> to VMobj.points'''
    ...
    def append_points(self, new_points):
        '''append points of paths<new_points> to VMobj.points'''
        # TODO, check that number new points is a multiple of 4?
        # or else that if len(self.points) % 4 == 1, then
        # len(new_points) % 4 == 3?
        self.points = np.append(self.points, self.validity_of_shape_input(
            new_points, test='appdpath'), axis=0)
        return self

    def add_subpath(self, points):  # (p,3)
        #assert(len(points) % 4 == 0)
        self.points = np.append(self.points, self.validity_of_shape_input(
            points, test='addpath'), axis=0)
        return self
    ...
    def add_cubic_bezier_curve(self, anchor1, handle1, handle2, anchor2):  # 1 path
        ...
        self.add_subpath([anchor1, handle1, handle2, anchor2])

    def add_cubic_bezier_curve_to(self, handle1, handle2, anchor):  # xpath
        '''append last 3 pathpoints<handle1, handle2, anchor>  to VMobj.points'''
        self.throw_error_if_no_points()
        # if not self.has_new_path_started():
        #    self.append_points([self.get_last_point()])
        self.append_points([handle1, handle2, anchor])

    # add Line
    def add_line_to(self, point):
        '''add line ctrls from anchor2<point>(3,) to VMobj.points
        ie line ctrls:handle1, handle2, anchor2'''
        #nppcc = self.n_points_per_cubic_curve
        self.append_points(self.alpha_pts([self.get_last_point(), point])[1:])
        return self

    def add_points_as_corners(self, points):
        '''add lines pts from anchor2s<points>(n, 3) to VMobj.points
        ie lines pts:[handle1, handle2, anchor2,*line,...]'''
        self.append_points(self.alpha_pts(np.append([self.get_last_point()], points, axis=0))[1:])
        return points

    def add_points_as_subpaths(self, points,length=3):
        '''add lines pts from subpaths<points>(n, p, 3) to VMobj.points
        ie lines pts:[anchor1, handle1, handle2, anchor2,*line,...]'''
        arry=np.moveaxis(points,0,-1)
        shape=arry.shape
        self.append_points(self.join01(self.alpha_pts(arry,length=len(shape),dim=shape[-1]).T))
        return self
    ...
    def create_arc_pts_from_corners(self, points, radius=0.5):
    ...
    def create_arc_pts_from_vertex(self, n, radius=0.5):
        arc = self.create_arc_pts_from_corners(
            self.get_vertices()[np.array([n-1, n, n+1])], radius=radius)
        return arc
        '''
        arcs.append(arc)
    ...
    def apply_matrix(self, matrix):
        return self.set_points(np.matmul(self.points, np.transpose(matrix)))
    ...
    if 1 == 1:
        ...
        def get_curve_points(self, n):
            ...
            if isinstance(n, int):
                n = [n]
            points = []
            [points.extend(self.get_nth_curve_points(i)) for i in n]
            return points
        ...
        def point_from_proportion(self, alpha):
            '''<from> means valid for regular geometry only'''
        ...
        def get_arc_center(self):
            ...
            if hasattr(self, 'points'):
                ...
                a1, h1, h2, a2 = [*self.points[:2],*self.points[-2:]]
                # Tangent vectors
                t1 = h1 - a1
                t2 = h2 - a2
                # Normals
                ...
                try:
                    ...
                    warnings.warn("aCan't find Arc center, using ORIGIN instead")
    ...
    if 1 == 1:
        ...
        def get_curve(self, n=0, copy=True, reverse=False):
            if copy:
                vmobj = self.copy()
            else:
                vmobj = self
            if isinstance(n, int):
                ...
                points = vmobj.get_curve_points(n)
            if reverse:
                points = np.flip(points, 0)
            return vmobj.set_points(points)

        def get_curves(self, n=None, copy=True):
            if n is None:
                n = range(self.get_num_curves())
            # elif isinstance(n, range):
            vgroup = VGroup(*[self.get_curve(i, True) for i in n])
            if copy:
                return vgroup
            else:
        ...
        def split_curve(self, ratio=None, copy=False, item=None):
            ...
            def checkmoj(var):
                return [(var.get_center()[0]-self.pts(0)[0])/(self.pts(1)[0]-self.pts(0)[0])]

            def checktuple(var):
                return [(var[0]-self.pts(0)[0])/(self.pts(1)[0]-self.pts(0)[0])]

            def checkvar(var):
                if isinstance(var, tuple):
                    return checktuple(var)
                elif isinstance(var, list):
                    tmp = []
                    [[tmp.append(i) for i in checkvar(each)] for each in var]
                    return tmp
                elif isinstance(var, (Mobject, VMobject)):
                    return checkmoj(var)
            ...
            ratio = checkvar(ratio)
            ratio.sort()
            vgroup = VMobject().set_points(split_bezier_points(
                self.get_points(), *ratio)).get_curves(item)
            # vgroup=VMobject().set_points(split_bezier_points(self.points,*ratio)).get_curves(item)

            vgroup.set_stroke(*stroke)
            if copy:
                return vgroup
            else:
                if len(vgroup) == 1:
                    return self.set_points(vgroup[0].points)
                else:
                    return self.become(vgroup)

    # Auxillary
    def get_copy(self,color=None,width=None,opacity=None):
        return self.copy().set_stroke(color,width,opacity)
    ...
    def init_vars(self):
        ...
        self.background_stroke_opacity = vars(
            self)['background_stroke_opacity']
...
class Line(VMobject):
    ...
    def zget_line(self, length=1, shift=None, color=None, width=None, opacity=None):
        #from manimlib.basic.basic_geometry import GeomLine
        if shift is None:
            shift = [0, 0, 0]
        elif isinstance(shift, (int, float)):
            shift = self.point_from_proportion(shift)
        return Line(ORIGIN, list(self.get_unit_vector())).scale_about_point(length, ORIGIN).shift(shift).set_stroke(color, width, opacity)

    def get_line(self, length=3, shift=None, stroke_color=None, stroke_width=None, stroke_opacity=None):
        from manimlib.basic.basic_geometry import GeomLine
        if shift is None:
            shift = [0, 0, 0]
        elif isinstance(shift, (int, float)):
...
class VLocation(VMobject):
    def __init__(self, mobject_or_point, direction=None, default=[0, 0, 0], **kwargs):
        super().__init__.find_xyz(mobject_or_point, direction, default)
